...
function generateExplanation(
  ...
  lines.push(`Это главный рубильник всей квартиры. Его номинал (${mainBreaker.rating}А) `
    ...
    + `Характеристика B — обязательное требование Ростехнадзора для вводных `
    + `автоматов жилых помещений: быстрее реагирует на короткое замыкание, `
    + `при этом спокойно держит пусковые токи бытовой техники `
  ...
  if (hasWet) {
    lines.push(`\nУ вас есть влажные помещения — для них нужна усиленная защита от утечки тока. `
      + `По ПУЭ во влажных зонах (ванная, туалет) сопротивление кожи резко падает, `
      + `и даже небольшой ток может быть опасен. По-хорошему здесь ставят 10мА УЗО — `
      + `это самый безопасный порог. Но на практике такие устройства — редкость `
      + `в продаже и стоят кратно дороже. При этом ПУЭ (п. 7.1.82) и `
      + `ГОСТ Р 50571.3-2009 прямо допускают для жилых помещений установку УЗО `
      + `с током утечки не более 30мА — это общепринятый стандарт безопасности, `
      + `утверждённый Ростехнадзором и проверенный десятилетиями. `
      + `Поэтому в расчёте используется именно 30мА — достаточно для защиты жизни, `
      + `полностью соответствует нормативам и реально купить в любом магазине.`)
  }
  ...
  if (strategy === 'economy') {
    ...
    if (waterDiffs.length > 0) {
      lines.push(`Кроме того, приборы, связанные с водой (стиральная, посудомоечная машина, `
        + `водонагреватель) вынесены на отдельные дифавтоматы 30мА. Это самое `
  ...
  if (waterLoads.length > 0) {
    ...
    for (const load of waterLoads) {
      ...
      lines.push(`**${label}**${room ? ` (${room.name})` : ''}. `
        ...
        + `защищены отдельным дифавтоматом 30мА. Это соответствует ПУЭ (п. 7.1.82) `
        + `и ГОСТ Р 50571.3-2009 — достаточный порог, чтобы спасти жизнь, `
        + `и при этом доступный в любом магазине электротоваров.`)
    }
  }
